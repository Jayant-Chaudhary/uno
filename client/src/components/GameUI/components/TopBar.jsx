...
const TopBar = ({
  ...
  if (desktop) {
    return (
      <div
        className={`md:flex hidden items-center justify-between px-6 py-3 rounded-2xl backdrop-blur-xl border transition-colors duration-500 ${
          isMyTurn
            ? "bg-green-950/40 border-green-500/50 shadow-[0_0_20px_rgba(34,197,94,0.1)]"
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <TurnTimer seconds={secondsLeft} />
          <div
            className={`px-5 py-2 rounded-xl font-black text-sm tracking-widest uppercase border ${
              isMyTurn
  ...
  return (
    <div>
      <div className="md:hidden flex items-center justify-between px-4 py-3 bg-black/40 backdrop-blur-md rounded-2xl border border-white/10 mb-3">
        {/* Turn label */}
        <div
          className={`px-3 py-1 rounded-full font-black text-sm tracking-wide ${
            isMyTurn
